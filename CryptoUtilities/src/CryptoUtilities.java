...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        if (!m.isZero()) {
            // divide n by m and store the result in temp
            NaturalNumber temp = n.divide(m);
            // copy the value of temp into n
            n.copyFrom(temp);
            // recursively call
            reduceToGCD(m, n);
            // transfer the value from m back to n
            n.transferFrom(m);
        }
    }
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        NaturalNumber temp = nCopy.divide(two);
        // if the result of the division is 0, then n is even
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p, NaturalNumber m) {
        ...
        if (p.compareTo(zero) > 0) {
            if (isEven(p)) {
                ...
                p.decrement();
                // store n for later
                NaturalNumber tempN = new NaturalNumber2();
                tempN.copyFrom(n);
                powerMod(n, p, m);
                // perform n % m
                NaturalNumber val = n.divide(m);
                n.transferFrom(val);
                // multiply n by the old n
                n.multiply(tempN);
                // perform n % m
                NaturalNumber valTwo = n.divide(m);
                n.transferFrom(valTwo);
                // restore p
                p.increment();
            }
        } else if (p.isZero()) {
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        boolean isPrime = true; // initialize isPrime
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            // handle odd numbers >= 5 using probabilistic method
            NaturalNumber nCopy = new NaturalNumber2(n);
            nCopy.decrement();
            final NaturalNumber one = new NaturalNumber2(1);
            final int fifty = 50;
            for (int i = 1; i < fifty; i++) {
                ...
                while (guess.compareTo(one) <= 0 || guess.compareTo(nCopy) >= 0) {
                    guess = randomNumber(nCopy);
                }
                // if it's composite, set result to false (it is not prime)
